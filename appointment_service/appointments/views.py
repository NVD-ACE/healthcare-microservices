...
# class AppointmentListView(APIView):
#     """
#     GET /api/appointments/?role=PATIENT&user_id=123
#     hoặc
#     GET /api/appointments/?role=DOCTOR&user_id=456
#
#     Không cần Authentication - để đơn giản hóa
#     """
#
#     def get(self, request):
#         try:
#             # Lấy role và user_id từ query params
#             role = request.query_params.get('role')
...
#
#             if not user_id:
#                 return Response({"error": "Phải cung cấp user_id trong query params"}, status=400)
#
#             # Lọc lịch hẹn dựa trên role và user_id
#             if role.upper() == 'PATIENT':
#                 qs = Appointment.objects.filter(patient_id=user_id)
#             elif role.upper() == 'DOCTOR':
#                 qs = Appointment.objects.filter(doctor_id=user_id)
